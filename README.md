...
- Boost – локальная версия библиотеки уже включена в репозиторий (`src/boost`), ничего дополнительно устанавливать не нужно
- Linux/MacOS/Windows (проект кроссплатформенный, но тестировался на Linux/MacOS)
...
Запустите игру из build-директории:

```sh
./ship-wars [-v]
```

Параметр `-v` (verbose): логгирование команд для изучения или дебаггинга.
